...
The detailed engineering log: what was built, what broke, what was measured,
and every bug fix along the way, with the evidence behind each claim. Entries
are roughly chronological (grouped by topic in places). For what's still open,
see [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md); for the current state at a glance,
the [README](../README.md).

## Docker: what the first real build caught

None of this was visible from
...
in [DEVELOPMENT.md](DEVELOPMENT.md)) — card detected on 30/30. Per-field accuracy (exact match after
normalizing case/whitespace, so this is a strict floor, not credit for
"close enough"):

*(Historical: this was the first measurement. Current numbers are in the

...
from app.max_body_size_middleware import MaxBodySizeMiddleware
...
def test_request_over_limit_rejected_via_content_length():
    # The common case: a client that reports Content-Length honestly is
    # rejected before the middleware even calls into the app, so the body
...
def test_request_over_limit_rejected_without_content_length():
    ...
    # Content-Length header, exercising the streaming byte-counter path
    # instead of the upfront header check.
    client = TestClient(_build_app(max_body_size=100))
